...
class Airport(gym.Env):
    """
    This consists of a series of m airports with n airplanes traveling across. At each time step, "new" cars enter the grid with
    ...
    ****"new" planes enter the grid at one of the first column of any row with probabiltiy p_arrive. However, the total number of 
    planes at any given time is limited to `Nmax`. *****
    ...
    Two planes collide if their locations overlap. A collision incurs a reward `rcoll = −10`, but does not affect
    the simulation in any other way. To discourage delays or stalling, each plane gets reward of `τ * r_time = −0.01τ`
    at every time step, where `τ` is the number of time steps passed since the plane arrived. Therefore, the total
    environment reward at time t is

    r(t) = C_t * r_coll + \sum_{i=1}_{N_t} {\tau_i * r_time}

    where C_t is the number of collisions occurring at time t and N_t is number of planes present. The simulation is
    terminated after 'max_steps(default:40)' steps and is classified as a failure if one or more collisions have
    occurred.

    Each plane is represented by one-hot binary vector set {n, l, r}, that encodes its unique ID, current location
    and assigned route number respectively. Each agent controlling a car can only observe other cars in its takeoff runway and its arrival runway,
    though low level communication is allowed in "v1" version of the game.
    ...
    We have m*(m-1) possible routes. m initial airports to leave from. There are (m-1) airports to arrive at. 
    ...
    def create_callsign(self):
        letters = ''.join(random.choices(string.ascii_uppercase, k=2))
        numbers = ''.join(random.choices(string.digits, k=3))
        # combine letters and numbers to create the flight call signal
        flight_call_signal = f"{letters}{numbers}"
        return flight_call_signal

    def get_data_stream(self, n=10):
        # create n flights across m airports. populate while their gate is open.
        airports_to_pick_from = ['SEA', 'SFO', 'MIA', 'ATL', 'DEN', 'JFK', 'ORD', 'DFW', 'EWR', 'PHX', 'LAS']
        info = []
        for i in range(n):
            sign = self.create_callsign()
            flight = [random.choice(airports_to_pick_from), random.choice(airports_to_pick_from)]
            while flight[0] == flight[1]:
                flight[1] = random.choice(airports_to_pick_from)
            info.append((sign, flight[0], flight[1]))
        return info
        



    def __init__(self, grid_shape=(6,7), step_cost=-0.01, n_max=2, collision_reward=-10, arrive_prob=0.5, m=6,
        ...
        self.input_data = self.get_data_stream()
        unique_airports = []
        for tup in self.input_data:
            if tup[1] not in unique_airports:
                unique_airports.append(tup[1])
            if tup[2] not in unique_airports:
                unique_airports.append(tup[2])
        n_flights = len(self.input_data)
        n_airports = len(unique_airports)
        self.num_states = 7 # gate -> taxi -> takeoff -> land -> taxi -> arrive, 7th state is placeholder/null
        self._grid_shape = (n_airports, self.num_states)
        self.airports = unique_airports

        # create dictionaries that correspond from airport to the row indx and vice versa
        self.row_to_airport = {}
        row = 0
        for a in self.airports:
            self.row_to_airport[row] = a
            row += 1
        
        self.airport_to_row = {}
        for elem in self.row_to_airport:
            self.airport_to_row[self.row_to_airport[elem]] = elem



        print(self.input_data)
        print(self._grid_shape)
        self.n_agents = n_flights
        self.m = self._grid_shape[0] # number of rows/airports
        m = self.m
        ...
        self.curr_planes_count = 0
        self.indx_to_name = {}

        #self._agent_view_mask = (3, 3) #CHANGE

        # entry gates where the planes spawn
        self._entry_gates = [(i, 0) for i in range(self.m)] #CHANGE 

        # destination places for the cars to reach
        self._destination = [(i, grid_shape[1]-1) for i in range(self.m)] #CHANGE 


        # TURNING PLACE: x-indx = 2


        # entire first column
        self._route_vectors = {i:(0,1) for i in self._entry_gates}


        self._agent_takeoff = [False for _ in range(self.n_agents)]  # flag if car changed direction
        self._agents_direction = [(0, 6) for _ in range(self.n_agents)] 
        ...
        # agent id (n_agents, onehot), obs_mask (m * 2), pos (r,c)
        mask_size = 2
        self._obs_high = np.ones((mask_size * (self.n_agents) + 2))  # 2 is for location
        self._obs_low = np.zeros((mask_size * (self.n_agents) + 2))  # 2 is for location
    ...
    def __init_full_obs(self):
        ...
        for agent_i in range(self.n_agents):
            if self.curr_planes_count >= len(self._entry_gates):
                self.agent_pos[agent_i] = (0, (self._grid_shape[1])-1)  # not yet on the road (6 -> NULL COLUMN)
                takeoff = (self.airport_to_row[self.input_data[agent_i][1]], 2) # predetermined takeoff
                landing = (self.airport_to_row[self.input_data[agent_i][2]], 3) # predetermined landing
                self.trajectories[agent_i] = (takeoff, landing)
            else:
                self.indx_to_name[(agent_i)] = self.input_data[agent_i][0]
                #pos = shuffled_gates[agent_i]
                takeoff = (self.airport_to_row[self.input_data[agent_i][1]], 2) # predetermined takeoff
                landing = (self.airport_to_row[self.input_data[agent_i][2]], 3) # predetermined landing
                self.trajectories[agent_i] = (takeoff, landing)
                # gets direction vector for agent_i that spawned in position pos
                pos = (self.airport_to_row[self.input_data[agent_i][1]], 0) # preset start
                self._agents_direction[agent_i] = self._route_vectors[pos]
                self.agent_pos[agent_i] = pos
                self.curr_planes_count += 1
    ...
    def get_agent_obs(self):
        ...
        agent_obs = []
        for agent_i in range(self.n_agents):
            mask_view = np.zeros((2, len(agent_no_mask_obs[0])), dtype=np.float32)
            coords = self.trajectories[agent_i]
            for coord in coords:
                row, col = coord
                if PRE_IDS['agent'] in self._full_obs[row][col]:
                    _id = int(self._full_obs[row][col].split(PRE_IDS['agent'])[1]) - 1
                    mask_view[0] = agent_no_mask_obs[_id] # Cannot collide in column three or four
    ...
    def __draw_base_img(self):
        ...
        for i, row in enumerate(self._full_obs):
            for j, col in enumerate(row):
                fill_cell(img, (i, j), cell_size=CELL_SIZE, fill=(255, 255, 255), margin=0.05)

        # make last column correspond to airports
        for i, row in enumerate(self._full_obs):
            for j, col in enumerate(row):
                if j == self._grid_shape[1]-1:
                    fill_cell(img, (i, j), cell_size=CELL_SIZE, fill=(143, 141, 136), margin=0.05)
                    write_cell_text(img, text=str(self.row_to_airport[i]), pos=(i, j), cell_size=CELL_SIZE,
    ...
    def step(self, agents_action):
        ...
        for agent_i, action in enumerate(agents_action):
            ...
            if not self._agent_dones[agent_i] and self._on_the_road[agent_i]:
                ...
                if collision_flag and self.agent_pos[agent_i][1] == 2 or self.agent_pos[agent_i][1] == 3:
        ...
        # adds new plane according to the probability _arrive_prob
        if random.uniform(0, 1) < self._arrive_prob:
            ...
            if not all(self._on_the_road) and free_gates:
               ...
               # self._agents_routes[agent_to_enter] = random.randint(1, self._n_routes)  # (1, 3) ... DO WE NEED THIS?
    ...
    def __update_agent_pos(self, agent_i, move):
        ...
        if move == 0:  # GAS
            if curr_pos[1] != 2: # check column isn't 2 (takeoff)
                next_pos = tuple([curr_pos[i] + self._agents_direction[agent_i][i] for i in range(len(curr_pos))])
            else: # make the jump since we are takeoff
                next_pos = (self.trajectories[agent_i][1][0], 3)
        elif move == 1:  # BRAKE
            pass
        else:
    ...
    def reset(self):
        ...
        #self._agent_turned = [False for _ in range(self.n_agents)] no longer needed
        self.curr_planes_count = 0
    ...
    def render(self, mode: str = 'human'):
        ...
        for agent_i in range(self.n_agents):
            if not self._agent_dones[agent_i] and self._on_the_road[agent_i]:
                ...
                write_cell_text(img, text=str(self.indx_to_name[(agent_i)]), pos=self.agent_pos[agent_i], cell_size=CELL_SIZE,
                                fill='white', margin=0.3)
